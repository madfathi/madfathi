...
{
sm->im[0]=IMG_Load("sous.png");
sm->im[1]=IMG_Load("SOUS1.png");
sm->im[2]=IMG_Load("sous4.png");
sm->im[3]=IMG_Load("mode.png");
sm->im[4]=IMG_Load("gf.png");
sm->im[5]=IMG_Load("gf1.png");
...
else if(event.motion.x>528 && event.motion.x<829 && event.motion.y >247  && event.motion.y <395 )
{
sm->a=2;
}
...
{
if(event.motion.x>534 && event.motion.x<814 && event.motion.y >221  && event.motion.y <350  )
{
sm->a=4;
}
if(event.motion.x>534 && event.motion.x<814 && event.motion.y >460  && event.motion.y <595 ) 
{
sm->a=5;
}
...
if(event1.type == SDL_MOUSEMOTION &&  sm->niveau == 1   )
{
sousmenucontrolmouse(event1,sm);
}
if(event1.type==SDL_KEYDOWN &&  sm->niveau==1 )
{
 ...
 if(event1.key.keysym.sym == SDLK_ESCAPE  )
    {
      ...
      //SDL_DEFAULT_REPEAT_INTERVAL;
       ...
       SDL_BlitSurface(m->im[0],NULL,window,NULL);
       SDL_Flip(window);
...
else if(event1.type == SDL_MOUSEMOTION && sm->niveau==2 )
{
sousmenucontrolmouse1(event1,sm);
}
else if(event1.type==SDL_KEYDOWN && sm->niveau==2)
{
...
{
if(event1.button.x>487 && event1.button.x<866 && event1.button.y >78 && event1.button.y <297  && sm->niveau==1 )
{
sm->a=3;
sm->niveau=2;
}
if(event1.button.x>533 && event1.button.x< 822 && event1.button.y >216 && event1.button.y <354  && sm->niveau==2 )
{
jeu1(window);
break;
}
if(event1.button.x>528 && event1.button.x<819  && event1.button.y >455 && event1.button.y <665  && sm->niveau==2 )
{
jeu(window);
}
...
{
SDL_Surface *photo=NULL,*masque=NULL,*photo1=NULL;
photo=IMG_Load("19.bmp");
photo1=IMG_Load("ml.png");
...
if(p.rect.x>=1382/4)
{
if(p.on_ground==0 && collisonparfaite(b.masque[b.k],p,b)==0)
{
...
if(saute==1)
{
collisonparfaite(b.masque[b.k],p,b)==0;
b.etatcamera1=1;
if(p.on_ground==0)
{
...
if(p1.rect.x>=1382*3/4)
{
if(p1.on_ground==0 && collisonparfaite(b.masque[b.k],p1,b)==0)
{
scrolling(&b,p1.direction,b.pasavancement);
}
else if(p.on_ground == 1)
{
...
if(p1.on_ground==0)
{
while(p1.saut.x<=50 && collisonparfaite(b.masque[b.k],p1,b)!=1)
{
b.rect2.x=b.rect2.x-6;
b.rect4.x=b.rect4.x-6;
b.rect5.x=b.rect5.x-6;
...
if(p1.on_ground==1)
{
while(collisonparfaite(b.masque[b.k],p1,b)!=1 && p1.rect.y<451 )
{
saut(&p1,masque,b);
animerbackground(&b);
scrolling(&b,8,b.pasavancement);
...
p1.saut.x=-50;
p1.position_saut=p1.rect.y;

}
if(p1.rect.y>450 )
{
p1.position_saut=450;
p1.rect.y=450;
p1.on_ground=0;
}
else if (p1.rect.y<450)
{
p1.on_ground=1;
}
...
SDL_GetMouseState(&x,&y);
SDL_WaitEvent(&event1);
SDL_BlitSurface(photo1,NULL,window,&rect);
SDL_Flip(window);

if(event1.type==SDL_MOUSEBUTTONDOWN)
{

if(event1.button.x>550 && event1.button.x<839  && event1.button.y > 285 && event1.button.y < 348  && x>550 && x<839  && y >285  && y <348  )
{
sousmenue=0;
}


if(x>550 && x<829  && y >505  && y <578 &&  event1.button.x>550 && event1.button.x<839  && event1.button.y > 511 && event1.button.y < 587 )
{
sousmenue=0;
continuer=1;
}
...
if (b.gamemode==1)
{
col=getpixel(calque,b.camera.x+775,p.rect.y+245);
...
if(b.etatcamera1==1)
{
col=getpixel(calque,b.camera1.x+430,p.rect.y+245);
if((col.r==0)&&(col.g==0) && (col.b==0))
return 1;
else
return 0;
...
if(b.etatcamera2==1)
{
col=getpixel(calque,b.camera2.x+430,p.rect.y+245);
if((col.r==0)&&(col.g==0) && (col.b==0))
return 1;
else
return 0;
...
time->t++;
sprintf(time->temp,"%d",time->t);
TTF_Init();
TTF_Font *font = TTF_OpenFont("angelina.TTF",68);
time->texte =TTF_RenderText_Solid(font,time->temp,color);

SDL_BlitSurface(time->texte,NULL,window,&time->rect1);
}
...
if(b->gamemode==2)
{
if(direction==0 && b->camera1.x<9666-1382-10 && b->etatcamera1==1 )
{
b->camera1.x+=pasavancement;
}
else if(direction==1 && b->etatcamera1==1)
{
b->camera1.x-=pasavancement;
...
if(direction==0 && b->camera2.x<9666-1382-10 && b->etatcamera2==1 )
{
b->camera2.x+=pasavancement;
}
else if(direction==1 && b->etatcamera2==1)
{
b->camera2.x-=pasavancement;
...
//m->font =TTF_OpenFont("angelina.TTF",50);
...
b->anime2 =IMG_Load("bird.png");

b->im1[0] =b->im[0] = IMG_Load("9999.png");
...
b->position1.x=0;
b->position1.y=0;
b->position1.w=1382/2;
b->position1.h=895;


b->position2.x=1382/2;
b->position2.y=0;
b->position2.w=1382/2;
b->position2.h=895;
...
if(b.k==0)
{
SDL_BlitSurface(b.im[b.k],&b.camera1,screen,&b.position1);
SDL_BlitSurface(b.im[b.k],&b.camera2,screen,&b.position2);
...
{
SDL_BlitSurface(b.im[b.k],&b.camera,screen,NULL);
SDL_BlitSurface(b.anime2,&b.rect3[b.i],screen,&b.rect6);
...
else if(p->direction == 8 && p->rect.x <=1382/2 )
{
